package com.yollock.resteasy.provider;
...
public class YolRestEasyJackson2Provider extends JacksonJaxbJsonProvider {

    protected static DeserializationConfig _deserializationConfig;
    ...
    static {
        SubtypeResolver _subtypeResolver = new StdSubtypeResolver();
        ...
        _deserializationConfig = _deserializationConfig.without(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES);

        // 禁止使用int代表Enum的order()來反序列化Enum,非常危险
        _deserializationConfig = _deserializationConfig.with(DeserializationFeature.FAIL_ON_NUMBERS_FOR_ENUMS);
        _deserializationConfig = _deserializationConfig.with(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY);
        // 输出的设置日期格式
        _deserializationConfig = _deserializationConfig.with(new SimpleDateFormat("yyyy-MM-dd hh:mm:ss"));
        // 空值转换
        _deserializationConfig = _deserializationConfig.with(DeserializationFeature.ACCEPT_EMPTY_STRING_AS_NULL_OBJECT);
    }
    ...
    public Object readFrom(Class<Object> type, //
        ...
        return reader.with(_deserializationConfig).withType(genericType).readValue(jp);
    }
